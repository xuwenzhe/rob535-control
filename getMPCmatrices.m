...
for i = 1:MPC_vars.N
    ...
    stage(i).Rk = costScale*2*diag([MPC_vars.rdDelta; MPC_vars.rFx; MPC_vars.rdVtheta]);
...
% quadratic rate input cost 
%stage(i).Rk = costScale*2*diag([MPC_vars.rdD; MPC_vars.rdDelta; MPC_vars.rdVtheta]);
stage(i).Rk = costScale*2*diag([MPC_vars.rdDelta; MPC_vars.rdFx; MPC_vars.rdVtheta]);
% linear state(-input) cost
stage(i).fk = costScale*generatef(traj,MPC_vars,ModelParams,Xk,i);
% linearized track constraints
...
function Qk = generateH(pathinfo,MPC_vars,ModelParams,Xk,i)
    ...
    Qk = 2*blkdiag(Qtilde,diag([MPC_vars.rDelta,MPC_vars.rFx, MPC_vars.rVtheta]));
    % scale cost
...
function [grad_eC, grad_eL] = getErrorGradient(pathinfo, theta_virt, ModelParams, x_phys, y_phys)
    ...
    grad_eC = [ sin_phi_virt, 0, -cos_phi_virt, zeros(1, ModelParams.nx-4), deC_dtheta];
    grad_eL = [-cos_phi_virt, 0, -sin_phi_virt, zeros(1, ModelParams.nx-4), deL_dtheta];
end
...
function f = generatef(pathinfo,MPC_vars,ModelParams,Xk,i)
    ...
    [eC, eL] = getErrors(pathinfo, Xk(end) ,x_phys,y_phys);
    e=[eC;eL];
    [grad_eC, grad_eL] = getErrorGradient(pathinfo, Xk(end), ModelParams, x_phys, y_phys);
