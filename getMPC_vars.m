...
if CarModel == "ORCA"
    ...
    MPC_vars.rFx = 1e-4;  % cost on Fx force
...
elseif CarModel == "FullSize"
    ...
    MPC_vars.Tx = diag(1./[1, 10, 1, 10, 2*pi, 5, 1]);
    MPC_vars.Tu = diag(1./[0.5,2.5e3,10]);

    MPC_vars.invTx = diag([1, 10, 1, 10, 2*pi, 5, 1]);
    MPC_vars.invTu = diag([0.5,2.5e3,10]);
...
%     MPC_vars.invTDu = eye(3);
    ...
    % MPC_vars.bounds = [-3,-3,-10,-0.1,-2,-7,   0,  -0.1,-0.35,0  ,  -1 ,-1,-5;
    %                     3, 3, 10,   4, 2, 7,  30,     1, 0.35,5  ,   1 , 1, 5]'; 
    %MPC_vars.bounds = [ 100, 0.25, -250, -2, -3, -1, 0,     -1, -2, 0,      -5, -5, -10;
                        %1600, 20,  900,   2, 3,  1,  1e4,    1,  1, 50,      5,  5,  10]'; 

    MPC_vars.bounds = [ 100, 0.25, -250, -1, -3, -0.05, 0,     -1, -2, 0,      -0.2, -0.1, -10;
                        1600, 2.5,  900,   1, 3,  0.05,  1e4,    1,  1, 2.5,      0.2,  0.1,  10]'; 
...
%     MPC_vars.bounds = [-1e4, 0.25, -1e4, -3,-3,-1,   0,    -1,-1, 0  ,  -0.1 ,-0.5,-10;
%                         1e4, 10,   1e4,  3, 3, 1, 1e4,     1, 1,10  ,   0.1 , 0.5, 10]'; 
    ...
    %% no obstacle
    %MPC_vars.qC = 0.010; % contouring cost   0.01 0.1
    %MPC_vars.qCNmult= 1000; % increase of terminal contouring cost
    %MPC_vars.qL= 1000; % lag cost 1000 1e4
    %MPC_vars.qVtheta= 0.5; % theta maximization cost
    %MPC_vars.qOmega = 5; % yaw rate regularization cost
    %MPC_vars.qOmegaNmult = 1; % yaw rate regularization cost
    %MPC_vars.rDelta= 1; % cost on steering 
    %MPC_vars.rVtheta= 0.01; % cost on virtual velocity
    %MPC_vars.rFx = 1e-8;  % cost on Fx force
    %MPC_vars.rdDelta= 1e3; % cost on change of steering 1e4
    %MPC_vars.rdFx= 1e0; % cost on change of duty cycle
    %MPC_vars.rdVtheta= 1e2; % cost on change of virtual velocity

    %% obstacle
    MPC_vars.qC = 1.0; % contouring cost   0.01 0.1
    MPC_vars.qCNmult= 10000; % increase of terminal contouring cost
    MPC_vars.qL= 10000; % lag cost 1000 1e4
    MPC_vars.qVtheta= 0.5; % theta maximization cost
    MPC_vars.qOmega = 5; % yaw rate regularization cost
    MPC_vars.qOmegaNmult = 1; % yaw rate regularization cost
    MPC_vars.rDelta= 1; % cost on steering 
    MPC_vars.rVtheta= 0.01; % cost on virtual velocity
    MPC_vars.rFx = 1e-8;  % cost on Fx force
    MPC_vars.rdDelta= 1e4; % cost on change of steering 1e4
    MPC_vars.rdFx= 1e0; % cost on change of duty cycle
    MPC_vars.rdVtheta= 1e2; % cost on change of virtual velocity
