...
%Tr=track.cline;%(track.outer+track.inner)/2;
%Xt=Tr(1,:);
%Yt=Tr(2,:);
% Tr = track.center;
% Xt = track.center(1,:);
% Yt = track.center(2,:);
[traj.ppx, traj.ppy, ~]=normalizedSplineInterp(track.cline(1,:),track.cline(2,:),1,'n');
...
[borders.pplx,borders.pply]=computeCenter(traj.ppx,traj.ppy,track.bl(1,:),track.bl(2,:),1);
[borders.pprx,borders.ppry]=computeCenter(traj.ppx,traj.ppy,track.br(1,:),track.br(2,:),1);

% [borders.pplx, borders.pply, ~ ]=normalizedSplineInterp(track.bl(1,:), track.bl(2, :),1,'n');
% [borders.pprx, borders.ppry, ~ ]=normalizedSplineInterp(track.br(1,:), track.br(2, :),1,'n');
