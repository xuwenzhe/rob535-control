...
% A = zeros((ng+50)*N,nz*N+nxu);
% b = zeros((ng+50)*N,1);
% index = 1;
% for i = 1:N
%     nObs = numel(stage(i+1).obB);
%     A(index,i*nz+[1:nxu]) = stage(i+1).Ck;
%     b(index) = stage(i+1).ug;
%     index = index+1;
%     
%     A(index,i*nz+[1:nxu]) = -stage(i+1).Ck;
%     b(index) = -stage(i+1).lg;
%     index = index+1;
% 
%     A(index:index+nObs-1,i*nz+[1:nxu]) = stage(i+1).obA;
%     b(index:index+nObs-1,1) = stage(i+1).obB;
%     index = index+nObs;
% end
