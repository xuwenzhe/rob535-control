...
% ppx, ppy are from center(traj), Xtr, Ytr are from boarder
...
%Xtr=[Xtr(1:end) Xtr(end)]; % wrap-around
%Ytr=[Ytr(1:end) Ytr(end)]; % wrap-around
t=zeros(nbreaks,1);
Xj=ppval(ppx,ppx.breaks); % at traj
Yj=ppval(ppy,ppx.breaks); % at traj

t(1)=ppx.breaks(1); %enforce start and end points are same
t(end)=ppx.breaks(end);
% t(end) is total length of traj
...
%    bla(i)=Pnext;
    index=Pprev+d*(Pnext-Pprev)/refine; % more precise way to interp
    t(i)=index;
